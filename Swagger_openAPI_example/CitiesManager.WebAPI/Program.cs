...
// .AddXmlSerializerFormatters();  //used if xml format data
...
builder.Services.AddSwaggerGen(options =>
{
    ...
    options.SwaggerDoc("v2", new Microsoft.OpenApi.Models.OpenApiInfo() { Title = "Cities web API", Version = "2.0" });
        ...
        ); //generates OpenAPI specification i. Documentation
...
app.UseSwagger(); // creates endponts for swagger.json
app.UseSwaggerUI(options =>
{
    options.SwaggerEndpoint("/swagger/v1/swagger.json", "1.0");
    options.SwaggerEndpoint("/swagger/v2/swagger.json", "2.0");
}); //create swagger UI for testing all web API endpoints
